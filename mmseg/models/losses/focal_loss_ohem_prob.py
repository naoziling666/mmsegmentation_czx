...
def sigmoid_focal_loss(pred,
    ...
    # tmp_target[~valid_mask[:,0]] = 0 # ingore pixel
    score = score.gather(1, tmp_target.unsqueeze(1)).squeeze(1)
    score, ind = score.contiguous()[valid_mask[:,0]].contiguous().sort()

    if iteartion <= 10000:
        min_kept = 1000000
        thresh = 1
        if score.numel() > 0:
            min_value = score[min(min_kept, score.numel() - 1)]
        else:
            return score.new_tensor(0.0)
        threshold = max(min_value, thresh)
        loss = loss[ind]
        loss = loss[score < threshold]
    elif 10000<iteartion<=20000:
        min_kept = 800000
        thresh = 0.8
        if score.numel() > 0:
            min_value = score[min(min_kept, score.numel() - 1)]
...
class FocalLoss_ohem_prob(nn.Module):
    ...
    def __init__(self,
                 ...
                 class_weight=None,
                 loss_weight=1.0,
                 loss_name='loss_focal_ohem_prob'):
        super().__init__()
