...
def parse_args():
    ...
    parser.add_argument('--config', help='the config path of model',default="/root/autodl-fs/mmsegmentation_czx/work_dirs/seafog_multiband/segnext_mscan-l_1xb4-adamw-40k_seafog_multiband9-600*600/20240801-085227/segnext_mscan-l_1xb4-adamw-40k_seafog_multiband9-600*600.py")
    parser.add_argument('--checkpoint', help='the checkpoint path of model', default="/root/autodl-fs/mmsegmentation_czx/work_dirs/seafog_multiband/segnext_mscan-l_1xb4-adamw-40k_seafog_multiband9-600*600/20240801-085227/iter_40000.pth")
    parser.add_argument('--images_path', help='the path of the image that requires inference', default="/root/autodl-fs/mmsegmentation_czx/data/seafog_data/seafog_multiband_600/image/val_no_overleap")
...
def main():
    ...
    for img in img_list:
        ...
        pred = inference_img_patch(model, image[0:600, 0:600, :9])
        # # 因为在训练时忽略了0的label
        # pred = pred+1
        # # use the mask of land
        # pred[land_gt==0] = 0

        # vis = np.zeros((600,600,3))
